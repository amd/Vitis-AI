...
class TokenizerImpl {
 ...
 private:
  ...
  using wordpiece_tokenizer_t = std::unique_ptr<WordPieceJsonTokenizer>;
  std::variant<bpe_tokenizer_t, ugm_tokenizer_t, wordpiece_tokenizer_t> tokenizer_;

  using bpe_decoder_t = std::unique_ptr<BpeStreamingDecoder>;
  using ugm_decoder_t = std::unique_ptr<SpmUgmDecoder>;
  using wordpiece_decoder_t = std::unique_ptr<WordPieceDecoder>;
  std::variant<bpe_decoder_t, ugm_decoder_t, wordpiece_decoder_t> detokenizer_;

  std::shared_ptr<llm_tokenizer::internal::TokenJsonConfig> tok_config_;
