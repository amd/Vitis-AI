...
struct KernelBpeTokenizer {
  ...
  using AddedTokenMap = llm_tokenizer::internal::AddedTokenMap;
  KernelBpeTokenizer(const BpeModelConf& conf);

  // Standalone C++ API (no ONNX Runtime dependencies)
 ...
 protected:
  ...
  std::unique_ptr<llm_tokenizer::internal::BpeModel> bbpe_tokenizer_;
...
class JsonFastTokenizer : public KernelBpeTokenizer {
 public:
  JsonFastTokenizer();
  OrtxStatus Load(const llm_tokenizer::internal::TokenJsonConfig& config);
  OrtxStatus LoadTikTokenBase64(const llm_tokenizer::internal::TokenJsonConfig& config);

 public:
  const auto& GetAddedTokens() const { return added_tokens_; }
  const llm_tokenizer::internal::BpeModel& GetEncoder() const { return *bbpe_tokenizer_; }
 ...
 private:
  ...
  void UpdateTokenizer(const llm_tokenizer::internal::TokenJsonConfig& config, const json& tok_json);
