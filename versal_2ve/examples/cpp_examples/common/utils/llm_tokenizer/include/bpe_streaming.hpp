...
class BpeStreamingDecoder : public KernelBpeDecoder {
 public:
  ...
  using TokenizerDecodingState = llm_tokenizer::internal::TokenizerDecodingState;

  // shared the data between the encoder and decoder
  OrtxStatus Load(
      std::shared_ptr<llm_tokenizer::internal::TokenJsonConfig const> ptr_config,
  ...
  OrtxStatus SpmId2Token(extTokenId_t id, std::string& token, bool& f_special_last) const {
    ...
    if (added_tokens_.count(id)) {
      ...
      std::string piece = (id >= 0 && static_cast<size_t>(id) < arr_vocab_.size()) ? arr_vocab_[id] : "";
      if (piece.empty()) {
        token = unk_token_;
        ...
        token = ReplaceAll(piece, std::string(llm_tokenizer::internal::spm_escaped_space), " ");
      }
    }
  ...
  OrtxStatus Id2Token(extTokenId_t id, std::string& token, TokenizerDecodingState** state, bool skip_special_tokens = true) const {
    ...
    std::unique_ptr<TokenizerDecodingState> bpe_state_ptr;
    bool is_first = false;
    if (bpe_state == nullptr) {
      bpe_state_ptr = std::make_unique<TokenizerDecodingState>();
 ...
 private:
  void CreateByteDecoder(const llm_tokenizer::internal::BpeModel& /* bpe_model */) {
    char32_t index = 256;
 ...
 private:
  ...
  std::shared_ptr<llm_tokenizer::internal::TokenJsonConfig const> tok_config_;
};
