 ...
 public:
  ...
  static void UpdateSpmByteToken(std::unordered_map<std::string, uint32_t>& vocab_map) {
    ...
    for (char32_t ch = 0; ch < 256; ++ch) {
      std::string tok(1, llm_tokenizer::internal::narrow<unsigned char>(ch));
  ...
  OrtxStatus Load(std::istream& vocab_stream, std::istream& merges_stream, const char* unk_token,
    ...
    if (it != vocab_map_.end()) {
      ...
      auto id = llm_tokenizer::internal::narrow<uint32_t>(vocab_map_.size());
    ...
    while (std::getline(merges_stream, line)) {
      ...
      int token_length = llm_tokenizer::internal::narrow<int>(w1.length() + w2.length());
    ...
    if (special_tokens != nullptr) {
      ...
      while (istrea >> line) {
        ...
        auto status = special_tokens_.Add(std::move(line_32), id);
        if (!status.IsOk()) {
          return status;
        }
      }
  ...
  OrtxStatus Load(const json& bpe_model, const json& tok_json, const char* /* special_tokens */, bool spm_converted) {
    auto status = LoadPreTokenizer(tok_json);
    if (!status.IsOk()) {
      return status;
    }
