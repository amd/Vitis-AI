...
struct SpmUgmTokenizer {
  ...
  using VocabTrieTree = llm_tokenizer::internal::TrieTree<char, extTokenId_t, -1>;
  using Vocab = std::unordered_map<std::string, std::tuple<extTokenId_t, double>>;

  // Helper struct for tokenization algorithm
  ...
  OrtxStatus Load(const TokenJsonConfig& config) {
    ...
    auto convert_hex_to_token = [](const std::string& str) {
      int val = -1;
      char char_3_4[3] = {0};
      // Check if string matches pattern exactly (not just byte length!)
      if (str.size() != val_pattern.size() || str[0] != '<' || str[1] != '0' || str[2] != 'x' || str[5] != '>') {
        return val;
      }
      // Extract hex digits at positions 3 and 4
      char_3_4[0] = str[3];
      char_3_4[1] = str[4];
    ...
    for (const auto& entry : vocab_node->items()) {
      // Use the array index as the token ID (vocab is an array where index = token ID)
      extTokenId_t token_id = std::stoi(entry.key());
      ...
      if (score != 0.0 || vocab_.count(tkn) == 0) {
        vocab_[tkn] = std::make_tuple(token_id, score);
      }
      // Track the max ID for later use (e.g., MarianTokenizer byte tokens)
      if (token_id >= id) {
        id = token_id + 1;
      }
    }
  ...
  // ONNX operator Compute method removed - use ComputeNoOp() for standalone C++ API
  ...
  std::pair<std::string_view, int> NmtNormalizePrefix(std::string_view input_view) const {
    ...
    if (longest_prefix_length > 0) {
      if (longest_prefix_offset >= prefix_replacements_size_) {
        throw std::runtime_error("[UgmTok]Index out of array bounds in precompiled charsmap!");
      }
   ...
   private:
    uint32_t GetNode(size_t index) {
      if (index >= xcda_array_size_) {  // Fix #2: off-by-one correction
        throw std::runtime_error("[UgmTok]Index out of array bounds in XCDA array!");
      }
...
class SpmUgmDecoder {
 public:
  ...
  OrtxStatus Load([[maybe_unused]] const TokenJsonConfig& config, const SpmUgmTokenizer& tokenizer) {
  ...
  // ONNX operator Compute method removed - use Id2Token() for standalone C++ API
  ...
  OrtxStatus Id2Token(extTokenId_t id, std::string& token, TokenizerDecodingState** state, [[maybe_unused]] bool skip_special_tokens /* only used by BPE; placeholder for UGM */ = true) const {
