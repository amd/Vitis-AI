...
enum class TokenType {
  kUnknown, kUnigram, kBPE, kWordPiece
};
...
constexpr std::pair<const char*, TokenType> kTokenizerDict[] = {
  ...
  {"BertTokenizer", TokenType::kWordPiece}
};
...
class TokenJsonConfig final {
 ...
 public:
  ...
  OrtxStatus Load(const std::string& json_path) {
    ...
    std::filesystem::path tok_dir(json_path);
    std::filesystem::path vocab_path(json_path);
    std::filesystem::path tok_path_obj(json_path);
    if (std::filesystem::is_directory(tok_path_obj)) {
      vocab_path = tok_dir / kDefaultVocabFile;
    } else {
      if (!std::filesystem::exists(tok_path_obj)) {
    ...
    if (json_config.find("chat_template") == json_config.end()) {
        ...
        std::ifstream jinjafs(jinja_path);
        if (jinjafs.is_open()) {
    ...
    if (std::filesystem::exists(module_cfg)) {
      std::ifstream module_ifs(module_cfg);
    ...
    if (!tiktoken_file.empty()) {
      ...
      if (std::filesystem::exists(tktok_path)) {
