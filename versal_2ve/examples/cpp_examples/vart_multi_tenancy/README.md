...
- **CPU Subgraph Support**: Runs models that contain CPU subgraphs at their
  input and/or output boundary by selecting the `CPU` tensor type per direction
...
`output_batch_size > 1`, all batch frames are concatenated into the same file per OFM
node. If `ofm-dir` is not specified in the JSON config, it defaults to the
current working directory (`"./"`).

Example output directory structure for a 2-model config with `"ofm-dir": "./outputs"`:
...
The only requirement is that the application select the correct tensor *type* for
...
Two optional per-model JSON fields select the type independently per direction:
...
The selected types are forwarded to the runner via the same options bag shown in
...
The example below runs two models spatially. `Model_1` has a CPU subgraph at its
input boundary (so it uses `input-tensor-type: "CPU"`) and an NPU output;
...
[
  {
    "model-cache-path": "/opt/models/resnet50/cache",
    "ifm-node-file-map": { "images_QuantizeLinear_Output": "/data/resnet50/inputs/image.bin" },
    "ofm-dir": "/data/resnet50/outputs"
  }
]
```
...
at any given moment. This is configured by setting `start-column` to the
**same** value and `aie-columns-sharing` to **`true`** for every model in the
group.
...
This is configured by giving each model a **different** `start-column` value
and setting `aie-columns-sharing` to **`false`**.
...
`aie-columns-sharing = false`. Models run truly in parallel on dedicated
...
- **`start-column`** — must be **distinct** per model (e.g. `0`, `4`, `8`,
