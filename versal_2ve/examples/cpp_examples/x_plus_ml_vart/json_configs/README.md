...
All other keys (`topk`, `label-file-path`, `post-process-print`, `nms-threshold`, `conf-threshold`, `model-params`, `grid-shape`, `anchors`, calibration parameters, etc.) are **type-specific** and are passed straight through to `vart::PostProcess`. The exact set required by each `type` is documented in the VART-X release's "Post Processing Function" reference.
