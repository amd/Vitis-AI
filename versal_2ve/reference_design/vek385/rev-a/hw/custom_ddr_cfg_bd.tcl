...
# ------------------------------------------------------------------------------
# 2026-08-11: DDR controller tuning for NoC_C1_C4 (C1+C2+C3+C4).
# Captured from Vivado GUI changes exported to 0811.tcl (per Srini):
#   - Interleave granularity = 256   (may later try 512 -> set INTERLEAVE_SIZE {512})
#   - Auto pre-charge OFF            (DDRMC5_AUTO_PRECHARGE false)
#   - Single-bank refresh            (DDRMC5_REFRESH_TYPE PER_BANK)
# Applied last so it overrides the base controller config set above. NoC_C0 is
# intentionally left unchanged.
# ------------------------------------------------------------------------------
startgroup
set_property -dict [list \
  CONFIG.DDRMC5_CONFIG(DDRMC5_INTERLEAVE_SIZE) {256} \
  CONFIG.DDRMC5_INTERLEAVE_SIZE {256} \
  CONFIG.DDRMC5_CONFIG(DDRMC5_AUTO_PRECHARGE) {false} \
  CONFIG.DDRMC5_CONFIG(DDRMC5_REFRESH_TYPE) {PER_BANK} \
] [get_bd_cells NoC_C1_C4]
endgroup
