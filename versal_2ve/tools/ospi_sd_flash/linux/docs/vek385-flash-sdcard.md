...
Automates UG1787 (SD Card Setup) for VEK385 Rev-A/Rev-B.
...
**On-target scripts (pre-installed via Yocto recipe):**

The following scripts are baked into the rootfs at build time via the `vek385-board-setup` Yocto recipe. They do not need to be in the script directory -- the rootfs already contains them:

- `setup_overlay.sh` -- programs FPGA PL overlay, copies config files
- `runtime_env.sh` -- loads kernel module, sets environment variables
- `configure_ufs.sh` + `ufsconfig_64gb` -- UFS flash check/enable
- `vek385-setup.service` -- systemd service that chains all setup steps on boot
- `/etc/profile.d/vek385-runtime.sh` -- environment variables for login shells

**Override (optional):** If these scripts are placed in the same directory as `vek385-flash-sdcard.sh`, the SD card script will copy them to the rootfs, overwriting the Yocto-baked versions. This allows customization without rebuilding the rootfs image.

If override scripts are not present, the Yocto-baked versions are used -- no manual setup needed after boot.
...
Step 5   mount /dev/sdc3 -> /mnt/vek385-sd/
         ...
         (if override scripts present in script dir:)
...
**From scripts directory (optional -- overrides Yocto-baked versions if present):**
...
These scripts are pre-installed in the rootfs via the `vek385-board-setup` Yocto recipe. If placed alongside the SD card script, they override the baked-in versions -- useful for testing customizations without rebuilding the rootfs.
...
`vek385-setup.service` (pre-installed via Yocto recipe) runs automatically as part of `multi-user.target` (after `local-fs.target`):
