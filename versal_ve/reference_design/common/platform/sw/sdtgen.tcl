...
#+---------------------------------------------------------------------------------------
#| RSH-5357 - TCL script for sdtgen utility (from Telluride repository)
#+---------------------------------------------------------------------------------------
for { set i 0 } { $i < $argc } { incr i } {
    ...
    # user dts
    if { [lindex $argv $i] == "-user_dts" } {
      incr i
      set user_dts [lindex $argv $i]
    }
  }
  ...
  # Use board_dts if provided, otherwise use user_dts
  if { [info exists board_dts] } {
    set_dt_param -board_dts $board_dts
  } elseif { [info exists user_dts] } {
    set_dt_param -user_dts $user_dts
  }
  generate_sdt
