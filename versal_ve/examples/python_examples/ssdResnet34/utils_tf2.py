...
def dboxes_R34_coco():
    ...
    steps=[(int(figsize[0]/fs[0]),int(figsize[1]/fs[1])) for fs in feat_size] ## SURESH The space b/w 2 adjacent anchor box center points for each predictor layer.
...
########## SURESH
        # Print statements
        #print(f"self.dboxes_xywh: {self.dboxes_xywh}")
        print(f"self.nboxes: {self.nboxes}") #15130
        print(f"self.scale_xy: {self.scale_xy}") #0.1
        print(f"self.scale_wh: {self.scale_wh}") #0.2
#########
    ...
    def scale_back_batch(self, bboxes_in, scores_in,device):
        ...
        """
       ## SURESH convert to (1,numBox,4) and (1,numBox,numClass) 
        bboxes_in = bboxes_in.transpose([0,2,1])
        scores_in = scores_in.transpose([0,2,1])

        bboxes_in[:, :, :2] = self.scale_xy*bboxes_in[:, :, :2] ## SURESH scales first two coordinates (left-top corner) 
        bboxes_in[:, :, 2:] = self.scale_wh*bboxes_in[:, :, 2:] ## SURESH scales the last two coordinates (right-bottom corner) 
...
########## SURESH
        print(f"Shape of bboxes_in before transposition: {bboxes_in.shape}") # (1,4,15130)
        print(f"Shape of scores_in before transposition: {scores_in.shape}") # (1,81,15130)
###########
    ...
    def decode_single(self, bboxes_in, scores_in, criteria, max_output, max_num=200):
        ...
        labels_out = np.array(labels_out, dtype=int)
